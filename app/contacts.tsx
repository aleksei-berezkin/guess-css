...
const rootClassLarge = css(({ theme }) => ({
    height: 'unset',
    '& a:not(:last-child)': {
...
export function Contacts(p: {large?: boolean, paragraph?: boolean}) {
    const rootClassName = `${ rootClass } ${ p.large ? rootClassLarge : '' }`
    const fontSize = p.large ? 'large' : 'medium';

    return <Typography color='textSecondary' className={ rootClassName }>
...
function Dev(p: {large?: boolean}) {
    const className = `${ devLogoClass } ${ p.large ? devLogoClassLarge : '' }`
